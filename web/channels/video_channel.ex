defmodule Scratch.VideoChannel do
  ...
  defp compute_additional_info(ann, socket) do
    for result <- Scratch.InfoSys.compute(ann.body, limit: 1, timeout: 10_000) do
      attrs = %{url: result.url, body: result.text, at: ann.at}
      info_changeset =
        Repo.get_by!(Scratch.User, username: result.backend)
        |> build_assoc(:annotations, video_id: ann.video_id)
        |> Scratch.Annotation.changeset(attrs)
